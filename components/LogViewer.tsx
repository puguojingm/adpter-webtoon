...
import { X, ChevronDown, ChevronRight, Clock, User, Bot, AlertTriangle } from 'lucide-react';
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
          <div className="flex items-center gap-2">
             ...
             <span className="px-2 py-0.5 bg-gray-200 text-gray-600 rounded-full text-xs font-mono">
               {logs.length} 记录
             </span>
          </div>
          <button 
             ...
             <div key={log.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
               ...
               >
                 <div className="flex items-center gap-4">
                    ...
                    <div className={`px-2 py-1 rounded text-xs font-bold ${log.status === 'PASS' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {log.status}
                    </div>
                    <div className="text-sm font-medium text-gray-800">
                       {log.type === 'breakdown' ? `剧情拆解 (批次 ${log.referenceId})` : `剧本生成 (第 ${log.referenceId} 集)`}
                    </div>
                 </div>
                 ...
                 <div className="border-t border-gray-100 bg-gray-50/50">
                    <div className="p-4 space-y-6">
                       ...
                       <div className="mt-4 pt-4 border-t border-gray-200">
                          ...
                          <div className={`p-3 rounded border text-sm whitespace-pre-wrap max-h-60 overflow-y-auto ${log.status === 'PASS' ? 'bg-green-50 text-green-800 border-green-100' : 'bg-red-50 text-red-800 border-red-100'}`}>
                              {log.result || "无结果输出"}
                          </div>
                       </div>
                    </div>
